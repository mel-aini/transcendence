...
const Table = ({isTournament}: {isTournament: boolean}) => {
	...
	return (
		<div ref={table} className="select-none relative w-full aspect-video">
			...
			<div ref={myPaddle} className="h-1/5 absolute -translate-y-1/2 rounded-full w-[2%]" style={{backgroundColor: profileState.userData?.game_settings.paddle, top: `${state.myPaddleData.y}%`, left: `${state.myPaddleData.x}%`}}/>

			<div ref={sidePaddle} className="h-1/5 absolute -translate-y-1/2 rounded-full w-[2%]" style={{backgroundColor: profileState.userData?.game_settings.paddle, top: `${state.sidePaddleData.y}%`, left: `${state.sidePaddleData.x}%`}}/>
